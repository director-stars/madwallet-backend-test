...
const swapRouterContractAddress = '0xe41f0FF3f4d90Bb1c4e32714532e064F9eA95F19';
const airswapWrapper = '0x6713c23261c8a9b7d84dd6114e78d9a7b9863c1a';
const zeroAddress = "0x0000000000000000000000000000000000000000";
const xendTokenAddress = "0x4a080377f83d669d7bb83b3184a8a5e61b500608";
...
const defaultFee = 50;
const chainId = 56;
const PROVIDER_URL = "https://speedy-nodes-nyc.moralis.io/"+MORALIS_KEY+"/bsc/mainnet";
...
class BscController {
    ...
    async trades ({ request, response }) {
        ...
        let priceResponse = await fetch(`https://deep-index.moralis.io/api/v2/erc20/${nativeToken}/price?chain=0x38`, {
            method: "GET",
            headers
        });
        ...
        if(queryData.sourceToken != zeroAddress){ 
            priceResponse = await fetch(`https://deep-index.moralis.io/api/v2/erc20/${queryData.sourceToken}/price?chain=0x38`, {
                method: "GET",
                headers
            });
        ...
        if(queryData.destinationToken != zeroAddress){
            priceResponse = await fetch(`https://deep-index.moralis.io/api/v2/erc20/${queryData.destinationToken}/price?chain=0x38`, {
                method: "GET",
                headers
            });
        ...
        airswapV3Data.hasRoute = false;
        airswapV3Data.fetchTime = 1000;
        ...
        airswapV3Data.approvalNeeded = approvalNeeded;
        ...
        airswapV3Data.gasMultiplier = parseFloat(dbTradeData.gasMultiplier);
        ...
        oneInchData.hasRoute = false;
        oneInchData.fetchTime = 1000;
        ...
        oneInchData.approvalNeeded = approvalNeeded;
        ...
        oneInchData.gasMultiplier = parseFloat(dbTradeData.gasMultiplier);
        ...
        paraswapData.hasRoute = false;
        paraswapData.fetchTime = 1000;
        ...
        paraswapData.approvalNeeded = approvalNeeded;
        ...
        paraswapData.gasMultiplier = parseFloat(dbTradeData.gasMultiplier);
        ...
        zeroExData.hasRoute = false;
        zeroExData.fetchTime = 1000;
        ...
        zeroExData.approvalNeeded = approvalNeeded;
        ...
        zeroExData.gasMultiplier = parseFloat(dbTradeData.gasMultiplier);
    ...
    async zeroExTradeData(web3, queryData, fee) {
        ...
        const tokenAmount = BigNumber(baseTokenAmount).mult(10000 - fee).div(10000);
        const feeAmount = BigNumber(baseTokenAmount).mult(fee).div(10000);
        const url = 'https://bsc.api.0x.org/swap/v1/quote?sellToken='+sourceTokenAddress+'&buyToken='+destinationTokenAddress+'&sellAmount='+tokenAmount+'&takerAddress='+queryData.walletAddress+'&slippagePercentage='+(queryData.slippage / 100)+'&skipValidation=true';
        console.log(url)
        let swapData;
        try {
            const res = await this.fetchWithTimeout(url, {
    ...
    async tokens ({ request, response }) {
        ...
        const queryResult = await BscToken.query().setVisible(['name', 'symbol', 'decimals', 'address', 'iconUrl', 'occurrences', 'aggregators']).orderBy('occurrences', 'desc').fetch();
    ...
    async token ({ request, response }) {
        ...
        const token = await BscToken.query().setVisible(['name', 'symbol', 'decimals', 'address', 'iconUrl', 'occurrences', 'aggregators']).where('address', address).first();
    ...
    async topAssets ({ request, response }) {
        const tokens = await BscToken.query().setVisible(['address', 'symbol']).where('topAsset', true).fetch();
        return tokens;
    }

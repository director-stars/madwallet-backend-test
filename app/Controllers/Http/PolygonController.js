...
const swapRouterContractAddress = '0x2F34767898CbCb2cd24F86AC4E61C785D49B2df7';
const airswapWrapper = '0x9c0f658331b9f87d7fa54ef0216689492a0176c5';
const zeroAddress = "0x0000000000000000000000000000000000000000";
const xendTokenAddress = "0x86775d0b80b3df266af5377db34ba8f318d715ec";
const nativeToken = "0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270";
const MORALIS_KEY = Env.get('MORALIS_KEY');
const MORALIS_X_API_KEY = Env.get('MORALIS_X_API_KEY');
const defaultFee = 50;
const chainId = 137;
const PROVIDER_URL = "https://speedy-nodes-nyc.moralis.io/"+MORALIS_KEY+"/polygon/mainnet";
...
class PolygonController {
    ...
    async trades ({ request, response }) {
        ...
        /////////////////////////////////
        let priceResponse = await fetch(`https://deep-index.moralis.io/api/v2/erc20/${nativeToken}/price?chain=0x89`, {
            method: "GET",
            headers
        });
        ...
        if(queryData.sourceToken != zeroAddress){
            priceResponse = await fetch(`https://deep-index.moralis.io/api/v2/erc20/${queryData.sourceToken}/price?chain=0x89`, {
                method: "GET",
                headers
            });
        ...
        if(queryData.destinationToken != zeroAddress){
            priceResponse = await fetch(`https://deep-index.moralis.io/api/v2/erc20/${queryData.destinationToken}/price?chain=0x89`, {
                method: "GET",
                headers
            });
    ...
    async zeroExTradeData(web3, queryData, fee) {
        ...
        const tokenAmount = BigNumber(baseTokenAmount).mult(10000 - fee).div(10000);
        const feeAmount = BigNumber(baseTokenAmount).mult(fee).div(10000);
        const url = 'https://polygon.api.0x.org/swap/v1/quote?sellToken='+sourceTokenAddress+'&buyToken='+destinationTokenAddress+'&sellAmount='+tokenAmount+'&takerAddress='+queryData.swapRouterContractAddress+'&slippagePercentage='+(queryData.slippage / 100)+'&skipValidation=true';
        let swapData;
        try {
            const res = await this.fetchWithTimeout(url, {
              timeout: 10000
    ...
    async tokens ({ request, response }) {
        ...
        const queryResult = await PolygonToken.query().setHidden(['id', 'updated_at', 'created_at']).orderBy('occurrences', 'desc').fetch();
    ...
    async token ({ request, response }) {
        ...
        const token = await PolygonToken.query().setVisible(['name', 'symbol', 'decimals', 'address', 'iconUrl', 'occurrences', 'aggregators']).where('address', address).first();
    ...
    async topAssets ({ request, response }) {
        const tokens = await PolygonToken.query().setVisible(['address', 'symbol']).where('topAsset', true).fetch();
        return tokens;
    }
